...
<body>
    ...
    if(isset($_GET["action"]) && $_GET["action"]=="view"){
        $appoid = $_GET["id"];
        $sqlmain = "SELECT appointment.*, doctor.docname, schedule.title, schedule.scheduledate, schedule.scheduletime,
                    (SELECT GROUP_CONCAT(CONCAT(disease, ':', diagnosis, ':', treatment, ':', prescription) SEPARATOR '||')
                     FROM patient_reports 
                     WHERE appointment_id=appointment.appoid) as report_data
                    FROM appointment 
        ...
        if($row){
            echo '<div class="popup-overlay">
                    <div class="popup-content">
                        ...
                        <div class="appointment-details">
                            ...
                            <p><strong>Doctor:</strong> Dr. '.$row["docname"].'</p>
                            <p><strong>Date:</strong> '.date("d-m-Y", strtotime($row["scheduledate"])).'</p>
                            <p><strong>Time:</strong> '.date("H:i", strtotime($row["scheduletime"])).'</p>';
            
            if($row["report_data"]) {
                $reports = explode("||", $row["report_data"]);
                foreach($reports as $report) {
                    list($disease, $diagnosis, $treatment, $prescription) = explode(":", $report);
                    echo '<div class="report-section" style="margin-top: 20px; padding: 15px; background: #f8f9fa; border-radius: 8px;">
                            <h3 style="color: #0083b0; margin-bottom: 15px;">Medical Report</h3>
            ...
            echo '</div>
                        <div style="text-align: center; margin-top: 20px;">
                            <a href="appointment.php" class="close-btn">Close</a>
                        </div>
                    </div>
                </div>';
        }
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            var viewButtons = document.querySelectorAll('.btn-view-appointment');
            viewButtons.forEach(function(button) {
                button.addEventListener('click', function() {
                    var appointmentId = this.getAttribute('data-id');
                    window.location.href = 'appointment.php?action=view&id=' + appointmentId;
                });
            });
        });
    </script>
